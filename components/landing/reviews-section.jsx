...
function StarRow() {
    return (<div className="flex items-center gap-1 text-[#f7d56b]">
      {Array.from({ length: 5 }).map((_, index) => (<Star key={index} className="h-3.5 w-3.5 fill-current"/>))}
    </div>);
}
function ReviewCard({ review }) {
    return (<article className="group min-w-[320px] max-w-[320px] lg:min-w-[360px] lg:max-w-[360px] rounded-[28px] border border-white/12 bg-white/[0.05] p-6 backdrop-blur-md shadow-[0_20px_80px_rgba(0,0,0,0.28)] transition-transform duration-300 hover:-translate-y-1 hover:border-white/22 hover:bg-white/[0.075]">
...
function ReviewTrack({ items, direction = "left" }) {
    const loopedItems = [...items, ...items];
    return (<div className="relative overflow-hidden">
      <div className={`flex w-max gap-5 py-4 ${direction === "left" ? "animate-marquee-left" : "animate-marquee-right"}`}>
        {loopedItems.map((review, index) => (<ReviewCard key={`${review.name}-${index}`} review={review}/>))}
      </div>
    </div>);
}
export function ReviewsSection() {
    ...
    return (<section id="reviews" className="relative py-28 lg:py-36 bg-background text-foreground overflow-hidden border-t border-background/10">
      <AsciiPattern />
