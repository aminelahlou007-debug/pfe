...
export function ChatbotWidget() {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[70] sm:bottom-6 sm:right-6">
      {isOpen ? (
        <aside className="transform transition-all duration-300 ease-in-out will-change-transform">
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    variant="ghost"
                    size="icon"
                    className="shrink-0 rounded-full text-white/90"
                    onClick={() => clearChat()}
            ...
            <CardContent className="p-0">
              <div className="max-h-[26rem] space-y-4 overflow-y-auto px-4 py-4">
                {messages.map((message, index) => (
                  <div key={`${message.role}-${index}`} className={cn("flex w-full items-end", message.role === "user" ? "justify-end" : "justify-start")}>
              ...
              <form onSubmit={sendMessage} className="border-t border-foreground/10 p-3 bg-background/80">
                <div className="flex items-end gap-2">
                  <Textarea
                    ...
                    placeholder="Ask about flowers, food, venues, tasks..."
                    className="min-h-12 resize-none rounded-2xl border-foreground/10 bg-background/80 text-sm"
                    rows={2}
                  />
                  <Button
                    type="submit"
                    size="icon"
                    className="h-11 w-11 rounded-2xl bg-foreground text-background hover:bg-foreground/90"
                    disabled={!canSend}
        ...
        <div className="flex items-center gap-3">
          <Button
            ...
            className="flex items-center gap-3 rounded-full bg-gradient-to-r from-foreground to-[#c77aa0] px-4 py-3 text-background shadow-2xl shadow-foreground/30 hover:scale-105 transform transition-transform"
          >
