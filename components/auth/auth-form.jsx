...
export function AuthForm() {
    ...
    const [role, setRole] = useState(searchParams.get("mode") === "admin" ? "admin" : "customer");
    ...
    return (<form onSubmit={handleSubmit} className="space-y-6 rounded-3xl border border-foreground/10 bg-card p-6 lg:p-8 shadow-xl">
      <div className="grid grid-cols-2 gap-2 rounded-full border border-foreground/10 bg-foreground/[0.03] p-1">
        {["customer", "admin"].map((item) => (<button key={item} type="button" onClick={() => handleRoleChange(item)} className={cn("rounded-full px-4 py-2 text-sm transition-colors", role === item ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground")}>
