...
export async function GET() {
    ...
    if (!collection)
        return NextResponse.json(listFallback("ceremonies"), { headers: noStoreHeaders() });
...
export async function POST(request) {
    ...
    if (!collection)
        return NextResponse.json(createFallback("ceremonies", parsed.data), { status: 201, headers: noStoreHeaders() });
