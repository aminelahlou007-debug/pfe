...
import { defaultVendors } from "@/lib/site-data";
...
export async function GET() {
    ...
    if (!collection)
        return NextResponse.json(listFallback("vendors"), { headers: noStoreHeaders() });
...
export async function POST(request) {
    ...
    if (!parsed.success)
        return jsonError("Invalid vendor data");
    if (!collection)
        return NextResponse.json(createFallback("vendors", parsed.data), { status: 201, headers: noStoreHeaders() });
