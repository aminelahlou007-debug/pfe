...
export async function GET() {
  ...
  if (!collection) return NextResponse.json(listFallback("guests"), { headers: noStoreHeaders() });
...
export async function POST(request: Request) {
  ...
  if (!collection) return NextResponse.json(createFallback("guests", parsed.data), { status: 201, headers: noStoreHeaders() });
