...
import { defaultTasks } from "@/lib/site-data";
...
export async function GET() {
    ...
    if (!collection)
        return NextResponse.json(listFallback("tasks"), { headers: noStoreHeaders() });
...
export async function POST(request) {
    ...
    if (!collection)
        return NextResponse.json(createFallback("tasks", parsed.data), { status: 201, headers: noStoreHeaders() });
