...
export default function CeremoniesPage() {
  ...
  useEffect(() => {
    const loadCeremonies = async () => {
      ...
      const data = await readJsonResponse<Ceremony[]>(response);
      setCeremonies(data ?? []);
      setIsLoading(false);
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1 max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            ...
            className="w-full h-10 pl-9 pr-4 bg-card border border-foreground/10 rounded-lg text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground/20"
          />
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {isLoading ? (
          <div className="col-span-full p-8 text-center text-muted-foreground">Loading events...</div>
        ) : (
          filteredCeremonies.map((ceremony) => (
            <div
              key={ceremony.id}
              className="group bg-card border border-foreground/10 hover:border-foreground/20 transition-all p-6"
            >
