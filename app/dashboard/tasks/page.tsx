...
export default function TasksPage() {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      ...
      const data = await readJsonResponse<Task[]>(response);
      setTasks(data ?? []);
      setIsLoading(false);
    };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          <div
            key={task.id}
            className={`group bg-card border border-foreground/10 hover:border-foreground/20 transition-all p-4 flex items-start gap-4 ${
