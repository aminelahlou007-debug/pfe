...
export async function getDashboardSummary() {
    ...
    if (!guests || !vendors || !tasks || !ceremonies) {
        const fallbackCeremonies = sortCeremonies(listFallback("ceremonies")).slice(0, 5).map(toUpcomingCeremony);
        return {
            stats: {
    ...
    return {
        ...
        upcomingCeremonies: sortCeremonies(upcoming).slice(0, 5).map(toUpcomingCeremony),
    };
}
